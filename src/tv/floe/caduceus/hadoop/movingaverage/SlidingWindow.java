 ...
 * 
 * Assumes incoming points were already sorted, throws exception if value is out
 * of order
 * 
 * Window is based on the delta between the timestamps on the front and back
 * points in the window, not the number of samples.
 * 
 * 
 * 
 */
public class SlidingWindow {
	...
	LinkedList<TimeseriesDataPoint> oCurrentWindow; // = new
													// LinkedList<Integer>();
	...
	public long GetWindowDelta() {
		...
		if (this.oCurrentWindow.size() > 0) {
			return this.oCurrentWindow.getLast().lDateTime
					- this.oCurrentWindow.getFirst().lDateTime
					+ this._lSampleSize;
		}
	...
	/**
	 * Slide the window forward - burn off the first half of the window - still
	 * must re-add more points from the Reduce iterator
	 * 
	 * @throws Exception
	 */
